(function() {
  ...
  function updateProfileDropdownState() {
    ...
    dropdownItems.forEach(item => {
        ...
        if (currentPath.includes(target) || (currentHash && currentHash.substring(1) === target)) {
    ...
    if (!Array.from(dropdownItems).some(item => item.classList.contains('active'))) {
        profileDropdown.classList.remove('active');
    }
}

// Panggil fungsi ini saat halaman dimuat
window.addEventListener('load', updateProfileDropdownState);

// Jika menggunakan navigasi single-page, panggil fungsi ini saat hash berubah
window.addEventListener('hashchange', updateProfileDropdownState);

// Untuk menangani scroll pada halaman index
function onScroll() {
    if (isIndexPage) {
        var scrollPosition = window.pageYOffset || document.documentElement.scrollTop;
        var sections = document.querySelectorAll('section');
...
// Panggil onScroll saat scroll dan saat halaman dimuat
if (isIndexPage) {
    window.addEventListener('scroll', onScroll);
    window.addEventListener('load', onScroll);
}
